...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .info-popover li { font-size: 0.82rem; color: var(--text); line-height: 1.45; }
    .info-popover a { color: var(--accent-h); text-decoration: none; }
    .info-popover a:hover { text-decoration: underline; }
    .info-popover-footer { font-size: 0.75rem; color: var(--muted); }
    ...
    /* ── Color picker pair ── */
    .color-pairs { display: flex; gap: 1.25rem; flex-wrap: wrap; }

    .color-pair { display: flex; flex-direction: column; gap: 5px; }

    .color-pair-label { font-size: 0.75rem; color: var(--muted); }

    .color-row { display: flex; align-items: center; gap: 8px; }

    /* Swatch wraps a native color input */
    .swatch {
      position: relative;
      width: 40px;
      height: 40px;
    ...
    .hex-in:focus { outline: none; border-color: var(--accent); }
    .hex-in.bad   { border-color: var(--red); }

    /* ── Page grid: left (preview + CSS) | right (sidebar config) ── */
    .page-grid {
      display: grid;
    ...
    .created-card a:hover { color: var(--accent-h); }
    .created-card img { height: 24px; width: auto; }
    ...
    .turn-dot.white {
      background: #f0ece4;
      box-shadow: 0 0 0 2px rgba(240,236,228,0.25);
    }
    .turn-dot.black {
      background: #2a2a2a;
    ...
    /* ── Output card ── */
    .output-card { flex: 1; display: flex; flex-direction: column; gap: 0.75rem; }
    ...
    textarea#css-out:focus { outline: none; border-color: var(--accent); }

    /* Custom scrollbar (webkit) */
    textarea#css-out::-webkit-scrollbar { width: 7px; }
    textarea#css-out::-webkit-scrollbar-track { background: transparent; }
    textarea#css-out::-webkit-scrollbar-thumb { background: var(--border); border-radius: 4px; }

    /* ── Overlay grid (single column in sidebar) ── */
    .ov-grid {
      display: flex;
      flex-direction: column;
      gap: 0.6rem;
    ...
    .ov-item { display: flex; flex-direction: column; gap: 0.45rem; }

    .ov-head { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.15rem; }

    .ov-name { font-size: 0.82rem; font-weight: 500; color: var(--text); white-space: nowrap; flex-shrink: 0; }
    ...
    .alpha-lbl { font-size: 0.78rem; color: var(--muted); width: 1rem; flex-shrink: 0; font-style: italic; }
    ...
    .alpha-slider::-moz-range-track { height: 6px; border-radius: 3px; }

    .alpha-val { font-family: "SF Mono","Fira Code",monospace; font-size: 0.78rem; color: var(--muted); min-width: 34px; text-align: right; }

    /* Full-width divider inside the overlay list */
    .col-divider {
      display: flex;
      align-items: center;
      gap: 0.75rem;
    ...
    .preset-item:hover .preset-thumb { border-color: var(--muted); }
    .preset-item.active .preset-thumb { border-color: var(--accent); box-shadow: 0 0 0 2px rgba(124,58,237,0.3); }
    .preset-name { font-size: 0.65rem; color: var(--muted); text-align: center; }
  ...
  /* ── Reset button ── */
    ...
    .reset-btn:hover  { background: var(--accent-h); }
  ...
  <style>
    /* Board background via ::before (base CSS has none — must declare here) */
    cg-board::before {
      content: '';
      position: absolute;
      inset: 0;
    ...
    piece.knight.white { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/wN.svg'); }
    piece.bishop.white { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/wB.svg'); }
    piece.rook.white   { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/wR.svg'); }
    piece.queen.white  { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/wQ.svg'); }
    piece.king.white   { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/wK.svg'); }
    piece.pawn.black   { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/bP.svg'); }
    piece.knight.black { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/bN.svg'); }
    piece.bishop.black { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/bB.svg'); }
    piece.rook.black   { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/bR.svg'); }
    piece.queen.black  { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/bQ.svg'); }
    piece.king.black   { background-image: url('https://raw.githubusercontent.com/lichess-org/lila/master/public/piece/cburnett/bK.svg'); }
  </style>
...
<div class="container">
  ...
  <header>
    <a href="/" class="header-title">
      <img src="/logo512.png" alt="Lichess Tailor logo" class="header-logo">
...
function makeTexturePattern(patId, txId, alphaPercent, sqSize, colour, prefix) {
  ...
  // The wrapper <g> then sets fill and stroke to the chosen colour so every
  // element inherits it, making the colour picker work for any SVG.
  const inner = svgSrc
    .replace(/^<svg[^>]*>/, '')
    .replace(/<\/svg>\s*$/, '')
